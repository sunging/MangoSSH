...
class PingConnectionTest {
    ...
    @Test
    fun `ping falls back to global request when server does not advertise ping`() = runTest {
        ...
        try {
            ...
            val authentication = backgroundScope.launch(dispatcher) {
                connection.authenticatePassword("ephemeral-user", java.util.UUID.randomUUID().toString())
            }
