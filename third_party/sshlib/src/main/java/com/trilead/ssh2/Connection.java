...
public class Connection implements AutoCloseable
{
	...
	private volatile TransportManager tm;
    ...
    public LocalStreamForwarder createLocalStreamForwarder(String host_to_connect, int port_to_connect) throws IOException {
        ...
        synchronized (this) {
            if (tm == null || !authenticated) throw new IllegalStateException("Connection is not authenticated");
            manager = cm;
        }
        return new LocalStreamForwarder(manager, host_to_connect, port_to_connect);
    }
	 ...
	 * @throws IOException on error
	 */
    /** Monotonic packet timestamps, zero until a packet has been observed. */
    public long getLastPacketSentNanos() { TransportManager current = tm; return current == null ? 0 : current.getLastPacketSentNanos(); }
    public long getLastPacketReceivedNanos() { TransportManager current = tm; return current == null ? 0 : current.getLastPacketReceivedNanos(); }
    ...
    public Session openSession() throws IOException {
        ...
        synchronized (this) {
            if (tm == null || !authenticated) throw new IllegalStateException("Connection is not authenticated");
            manager = cm;
            random = getOrCreateSecureRND();
        }
        return new Session(manager, random);
    }
    ...
    /** MangoSSH: response deadline without holding the connection monitor during I/O. */
    private static final java.util.concurrent.ScheduledExecutorService PROBE_DEADLINES =
        java.util.concurrent.Executors.newScheduledThreadPool(2, runnable -> {
            Thread thread = new Thread(runnable, "MangoSSH-ProbeDeadline");
            thread.setDaemon(true);
            return thread;
        });

    /** MangoSSH: the deadline includes queued global requests and a blocked socket write. */
    public void ping(long timeoutMillis) throws IOException {
        if (timeoutMillis <= 0) throw new IllegalArgumentException("Invalid probe timeout");
        ChannelManager current;
        TransportManager transport;
        synchronized (this) {
            if (cm == null || !authenticated) throw new IOException("SSH connection is not ready");
            current = cm;
            transport = tm;
        }
        java.util.concurrent.atomic.AtomicBoolean settled = new java.util.concurrent.atomic.AtomicBoolean();
        java.util.concurrent.ScheduledFuture<?> deadline = PROBE_DEADLINES.schedule(() -> {
            if (settled.compareAndSet(false, true))
                transport.close(new java.net.SocketTimeoutException("SSH probe timed out"), false);
        }, timeoutMillis, java.util.concurrent.TimeUnit.MILLISECONDS);
        try {
            current.requestGlobalTrileadPing(timeoutMillis);
            if (!settled.compareAndSet(false, true)) throw new java.net.SocketTimeoutException("SSH probe timed out");
        } catch (IOException failure) {
            transport.close(failure, false);
            throw failure;
        } finally {
            settled.set(true);
    ...
    /** MangoSSH: force wakes transport I/O even when a synchronized operation is blocked. */
    public void abort() {
        TransportManager current = tm;
        if (current != null) current.close(new IOException("SSH transport stopped"), false);
    }
