...
public class TransportManager implements ITransportConnection
{
    ...
    private volatile long lastPacketSentNanos;
    private volatile long lastPacketReceivedNanos;
    public long getLastPacketSentNanos() { return lastPacketSentNanos; }
    public long getLastPacketReceivedNanos() { return lastPacketReceivedNanos; }
	...
	{
        Socket firstObserved = sock;
        Socket finalObserved;
			...
			if (!connectionClosed)
			{
				if (useDisconnectPacket)
				{
					try
					{
						...
						if (tc != null)
							tc.sendMessage(msg);
                lastPacketSentNanos = System.nanoTime();
					}
					catch (IOException ignore)
					{
					}
        ...
        // A connecting thread may have adopted its socket between the first observation and the close marker.
        if (!useDisconnectPacket && finalObserved != null && finalObserved != firstObserved) {
            try { finalObserved.close(); } catch (IOException ignore) { }
        }
	...
	{
        Socket opened = proxyData == null ? connectDirect(hostname, port, connectTimeout, ipVersion)
            : proxyData.openConnection(hostname, port, connectTimeout);
        boolean accepted;
        synchronized (connectionSemaphore) {
            accepted = !connectionClosed;
            if (accepted) sock = opened;
        }
        if (!accepted) {
            opened.close();
            throw new IOException("Connection closed during socket creation");
        }
	}
